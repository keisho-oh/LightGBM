 ...
 */
#ifndef LightGBM_METRIC_WEIGHTED_RANK_METRIC_HPP_
#define LightGBM_METRIC_WEIGHTED_RANK_METRIC_HPP_
...
class WeightedNDCGMetric:public Metric {
 public:
  explicit WeightedNDCGMetric(const Config& config) {
    // get eval position
    eval_at_ = config.eval_at;
    auto label_gain = config.label_gain;
    WeightedDCGCalculator::DefaultEvalAt(&eval_at_);
    WeightedDCGCalculator::DefaultLabelGain(&label_gain);
    // initialize DCG calculator
    WeightedDCGCalculator::Init(label_gain);
  }
  ...
  void Init(const Metadata& metadata, data_size_t num_data) override {
    for (auto k : eval_at_) {
      name_.emplace_back(std::string("weighted_ndcg@") + std::to_string(k));
    }
    num_data_ = num_data;
    // get label
    label_ = metadata.label();
    num_queries_ = metadata.num_queries();
    WeightedDCGCalculator::CheckMetadata(metadata, num_queries_);
    WeightedDCGCalculator::CheckLabel(label_, num_data_);
    ...
    for (data_size_t i = 0; i < num_queries_; ++i) {
      ...
      WeightedDCGCalculator::CalMaxDCG(eval_at_, label_ + query_boundaries_[i],
                               theta1_ + query_boundaries_[i],
                               theta2_ + query_boundaries_[i],
    ...
    if (query_weights_ == nullptr) {
      ...
      for (data_size_t i = 0; i < num_queries_; ++i) {
        ...
        if (inverse_max_dcgs_[i][0] <= 0.0f) {
          ...
          // calculate DCG
          WeightedDCGCalculator::CalDCG(eval_at_, label_ + query_boundaries_[i],
...
#endif   // LightGBM_METRIC_WEIGHTED_RANK_METRIC_HPP_
